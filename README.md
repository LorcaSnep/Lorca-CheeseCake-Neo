...
## Status:
As of 6/16/26 I am awaiting delivery of V4.5.  This version is a small revision on the previous version which fixes the status LED and has some minor tweaks with the buck converter section.  The previous version is functioning with the latest version of SlimeVR firmware installed and I have been using it for several months.  The next step is to design the enclosure for the AUX modules.  I intend for these to be rather simple 3D printed assemblies to be fastened with velcro as opposed to the full strap due to the small size and weight.  Additionally, every required 3D print file will be uploaded to this project to have all the required components for this project in one location.
